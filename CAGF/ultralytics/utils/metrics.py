...
def xywh2xyxy(box):
    ...
    xy = box[:, :2]
    wh = box[:, 2:] / 2
    return torch.cat([xy - wh, xy + wh], dim=1)


def focaler_giou_loss(box1, box2, xywh=True, d=0.2, u=0.9, eps=1e-7):
    """
    Compute Focaler-GIoU loss based on the paper definition.

    Args:
        box1: predicted boxes, shape (N, 4)
        box2: target boxes, shape (N, 4)
        xywh: whether input is in xywh format
        d, u: interval bounds for Focaler-IoU
    ...
    # Step 2: Calculate IoU and enclosing area
    ...
    # focaler iou
    iou_focal = torch.where(iou < d, torch.zeros_like(iou),
                    torch.where(iou > u, torch.ones_like(iou),
                        (iou - d) / (u - d)))

    # L = 1 - GIoU + IoU - IoU_focaler
    loss = l_giou + iou - iou_focal
    return loss.mean()
