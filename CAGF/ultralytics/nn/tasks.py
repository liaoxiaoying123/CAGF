...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        #新添加Drone-detr论文里的模块  
        # elif m in {EDF_FAM}:
        #     c1 = [ch[x] for x in f]  # 支持多个输入
        #     c2 = c1[0]              # 默认输出通道等于第一个输入分支的通道数，可按需要调整
        #     args = [c1] + args      # 构造参数，将输入通道列表作为第一个参数传入
        elif m in {EDF_FAM}:
            # 获取两个输入层的索引
            idx1, idx2 = f[0], f[1]
            
            # 获取通道数（处理可能的列表形式）
            c1 = ch[idx1] if isinstance(ch[idx1], int) else ch[idx1][0]
            c2 = ch[idx2] if isinstance(ch[idx2], int) else ch[idx2][0]
            
            # 验证输入通道数相同
            if c1 != c2:
                raise ValueError(f"EDF_FAM requires same input channels. Got {c1} and {c2}")
            ...
            # 直接返回模块实例
            m_ = module
        elif m in {FastResidual1, FastResidual2}:
            # 获取输入通道数
            c1 = ch[f]
            ...
            if isinstance(c1, list):
                if len(c1) == 1:
                    ...
                    raise ValueError(f"{m.__name__} requires single input, got {len(c1)} inputs")
        ...
        elif m in {DySnakeConv, SPDConv, DCNv2, BasicStage}:
            c1, c2 = ch[f], args[0]
            args = [c1, c2, *args[1:]]  
        ...
        elif m in {Blocks2, Blocks3}:
            # Blocks2(args[0]=planes, args[1]=block_type)
